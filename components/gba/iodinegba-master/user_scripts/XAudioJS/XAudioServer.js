...
export class XAudioServer extends Component {
    constructor(channels, sampleRate, minBufferSize, maxBufferSize, underRunCallback, heartbeatCallback, postheartbeatCallback, volume, failureCallback, userEventLatch, props) {
        super(props);
...
export function XAudioJSFlashAudioEvent() {		//The callback that flash calls...
...
export function XAudioJSGenerateFlashSurroundString() {	//Convert the arrays to one long string for speed.
...
export function XAudioJSGenerateFlashStereoString() {	//Convert the arrays to one long string for speed.
    var XAudioJSTotalSamples = XAudioJSSamplesPerCallback << 1;
    if (XAudioJSBinaryString.length > XAudioJSTotalSamples) {
        XAudioJSBinaryString = [];
    }
...
export function XAudioJSGenerateFlashMonoString() {	//Convert the array to one long string for speed.
...
export function XAudioJSWebAudioEvent(event) {		//Web Audio API callback...
    if (XAudioJSWebAudioWatchDogTimer) {
        XAudioJSWebAudioWatchDogLast = (new Date()).getTime();
    }
...
export function XAudioJSResampleRefill() {
...
export function XAudioJSCallbackAPIEventNotificationCallbackCompatTimerClear() {
...
export function XAudioJSGetArraySlice(buffer, lengthOf) {
